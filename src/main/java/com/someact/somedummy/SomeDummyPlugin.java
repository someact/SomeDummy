    ...
    public void onEnable() {
        ...
        if (!configManager.isSilentStartup()) {
            Bukkit.getConsoleSender().sendMessage(com.someact.somedummy.util.MessageUtil.parse(
                    "<dark_gray>[<gradient:#ff7675:#fab1a0><bold>SomeDummy</bold></gradient>]</dark_gray> <green>Thank you for using my <gradient:#ff7675:#fab1a0><bold>SomeDummy</bold></gradient> plugin! :></green> <dark_gray>(v" + getPluginMeta().getVersion() + ", " + (System.currentTimeMillis() - startTime) + "ms)</dark_gray>"
            ));
        } else {
